...
class Userprofile(models.Model):
    ...
    user = models.OneToOneField(User, on_delete=models.CASCADE, related_name="userprofile", verbose_name=_("user"))
    ...
    language = models.CharField(
        _("language"), max_length=10, default="", blank=True, null=False, help_text=_("Preferred language")
    )
...
class UserCard(models.Model):
    ...
    order = models.IntegerField(
        _("order"), help_text=_("What order to display this card within the profile dashboard."), default=1
    )
    ...
    def __str__(self):
        return "{} - {} (o:{})".format(self.profile, self.card, self.order)
...
class Table(models.Model):
    ...
    default_fields = models.ManyToManyField(
        "TableColumn", blank=True, related_name="default_field", verbose_name=_("default fields")
    )

    owner = models.ForeignKey(User, on_delete=models.CASCADE, verbose_name=_("owner"))
    last_edit_date = models.DateTimeField(_("last edit date"), null=True, blank=True, db_index=True)
    ...
    table_type = models.CharField(_("type"), max_length=1, default=TYPE_REGULAR, null=False, blank=True, choices=TYPES)
    filters = models.JSONField(verbose_name=_("filters"), encoder=DjangoJSONEncoder, null=True, blank=True)

    class Meta:
        permissions = (("update_content", _("Can update table content")),)
...
class TableColumn(models.Model):
    ...
    table = models.ForeignKey("Table", on_delete=models.CASCADE, related_name="fields", verbose_name=_("table"))
    ...
    choices = ArrayField(models.CharField(max_length=100), null=True, blank=True, verbose_name=_("choices"))
...
class CsvFieldMap(models.Model):
    ...
    field_type = models.CharField(
        _("field type"), max_length=20, choices=datatypes, default=datatypes[0][0], null=True, blank=True
    )
    field_format = models.CharField(_("field format"), max_length=20, null=True, blank=True)
    required = models.BooleanField(_("required"), default=False)
    unique = models.BooleanField(_("unique"), default=False)
    table_column = models.ForeignKey(
        "TableColumn", null=True, blank=True, on_delete=models.CASCADE, verbose_name=_("table column")
    )
...
class CsvImport(models.Model):
    ...
    delimiter = models.CharField(_("delimiter"), max_length=2, default=";", null=True, blank=True)
    table = models.ForeignKey(
        "Table",
...
class Entry(models.Model):
    ...
    table = models.ForeignKey("Table", on_delete=models.CASCADE, related_name="entries", verbose_name=_("table"))
...
class FilterJoinTable(models.Model):
    ...
    fields = models.ManyToManyField(TableColumn, related_name="filter_join_table_fields", verbose_name=_("fields"))
...
class TableLink(models.Model):
    entry = models.ForeignKey(Entry, null=False, on_delete=models.CASCADE, verbose_name=_("entry"))
    entry_field = models.ForeignKey(
        TableColumn, null=False, on_delete=models.CASCADE, related_name="+", verbose_name=_("entry field")
    )
    target_field = models.ForeignKey(
        TableColumn, null=False, on_delete=models.CASCADE, related_name="+", verbose_name=_("target field")
    )
    created_on = models.DateTimeField(auto_now_add=timezone.now, editable=False, verbose_name=_("created on"))
...
class Filter(models.Model):
    ...
    primary_table = models.ForeignKey(
        FilterJoinTable, null=True, on_delete=models.CASCADE, verbose_name="primary table"
    )
    join_tables = models.ManyToManyField(
        FilterJoinTable, related_name="filter_join_table", verbose_name=_("join tables")
    )
    filters = models.JSONField(_("filters"), encoder=DjangoJSONEncoder, null=True, blank=True)
    creation_date = models.DateTimeField(_("creation date"), auto_now_add=timezone.now, null=True, db_index=True)
    default_fields = models.ManyToManyField(
        TableColumn, related_name="filter_default_field", blank=True, verbose_name=_("default fields")
    )
...
class Chart(models.Model):
    ...
    chart_type = models.CharField(_("chart type"), max_length=20, default=chart_types[0][0], choices=chart_types)
    table = models.ForeignKey(Table, on_delete=models.CASCADE, verbose_name=_("table"))
    ...
    timeline_period = models.CharField(
        ...
        null=True,
        blank=True,
        choices=chart_timeline_periods,
        default=chart_timeline_periods[0][0],
    )
    ...
    x_axis_field_2 = models.ForeignKey(
        ...
        blank=True,
        on_delete=models.SET_NULL,
        related_name="charts_x_axis_fields_group",
    ...
    y_axis_function = models.CharField(
        _("y axis function"), max_length=10, default=chart_functions[0][0], choices=chart_functions
    )
...
class Card(models.Model):
    ...
    data_column_function = models.CharField(
        _("data column function"), max_length=10, default=chart_functions[0][0], choices=chart_functions
    )
...
class PluginTaskResult(models.Model):
    ...
    status = models.CharField(_("status"), max_length=11, default=RUNNING, choices=STATUS_CHOICES, db_index=True)
    ...
    user = models.ForeignKey(
        User, null=True, on_delete=models.SET_NULL, related_name="%(app_label)s_%(class)s_tasks", verbose_name=_("user")
    )
    success = models.BooleanField(_("success"), default=False)
    stats = models.JSONField(_("stats"), encoder=DjangoJSONEncoder, null=True, blank=True)
