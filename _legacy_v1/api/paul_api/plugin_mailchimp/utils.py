...
from api.models import Table, Database, TableColumn, Entry
...
def create_mailchimp_tables(audiences_name: str = "") -> int:
    ...
    contact_table = get_or_create_table(audiences_name, "contact_merge_fields", "audience_members")
    contact_table.table_type = Table.TYPE_CONTACTS
    contact_table.save()

    get_or_create_table(mc_settings.segment_members_table_name, "contact_merge_fields", "segment_members")
...
def get_or_create_table(table_name: str, *table_rulesets: str) -> Table:
    ...
    table, created = Table.objects.get_or_create(name=table_name, database=db, owner=user)
    ...
    if created:
        ...
        for field_name, field_details in table_fields_defs.items():
            column, created = TableColumn.objects.get_or_create(table=table, name=field_name)
...
def retrieve_lists_data(client: MailChimp):
    ...
    try:
        ...
        return (False, {"details": [_("Could not connect to mailchimp. Check API Key.")]})
    ...
    for mlist in lists["lists"]:
        audience_exists = Entry.objects.filter(table=audiences_table, data__id=mlist["id"])
        ...
        if audience_exists:
            ...
            audience_entry = Entry.objects.create(table=audiences_table, data={"id": mlist["id"]})
        ...
        # Sync list stats
        audience_stats_exists = Entry.objects.filter(table=audiences_stats_table, data__audience_id=mlist["id"])
        if audience_stats_exists:
            ...
            audience_stats_entry = Entry.objects.create(
                table=audiences_stats_table, data={"audience_id": mlist["id"], "audience_name": mlist["name"]}
            )
        ...
        for segment in list_segments["segments"]:
            ...
            if audience_segments_exists:
                ...
                audience_segments_entry = Entry.objects.create(
                    ...
                    data={"audience_id": segment["list_id"], "audience_name": mlist["name"]},
                )
            ...
            segment_members = client.lists.segments.members.all(
                list_id=mlist["id"], segment_id=segment["id"], get_all=True
            )
        ...
        for member in list_members["members"]:
            ...
            if audience_members_exists:
                ...
                audience_members_entry = Entry(
                    ...
                    data={"audience_id": member["list_id"], "audience_name": mlist["name"]},
                )
...
def get_emails_from_filtered_view(token, filtered_view):
    ...
    while continue_request:  # TODO: get rid of web request
        url = "http://{}/api/filters/{}/entries/?page={}".format(settings.ALLOWED_HOSTS[0], filtered_view.pk, page)
