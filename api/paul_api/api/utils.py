...
from django.db.models import Avg, CharField, Count, DateTimeField, FloatField, Max, Min, Q, Sum
...
def get_database_values(table, field, batch_values):
    ...
    """
    return list(
        models.Entry.objects.filter(table=table)
        .filter(**{"data__{}__in".format(field): batch_values})
        .distinct()
        .values_list("data__{}".format(field), flat=True)
    )
...
def import_csv(reader, table, csv_import=None):
    ...
    unique_fields = {
        field_map.table_column.name: field_map.original_name
        for field, field_map in csv_field_mapping.items()
...
def get_chart_data(request, chart, table, preview=False):
    ...
    chart_data = models.Entry.objects.filter(table=chart.table).filter(filter_dict)
    if preview:
        chart_data = chart_data[:100]

    if chart.timeline_field:
        chart_data = (
            chart_data.annotate(date_field=Cast(KeyTextTransform(chart.timeline_field.name, "data"), DateTimeField()))
            .annotate(time=Trunc("date_field", chart.timeline_period.lower(), is_dst=False))
            .values("time")
        )
    else:
        chart_data = chart_data.annotate(series=Cast(KeyTextTransform(chart.x_axis_field.name, "data"), CharField()))
        if chart.x_axis_field_2:
            chart_data = chart_data.annotate(
                series_group=Cast(KeyTextTransform(chart.x_axis_field_2.name, "data"), CharField())
    ...
    if chart.y_axis_field:
        chart_data = chart_data.annotate(
            value=y_axis_function(Cast(KeyTextTransform(chart.y_axis_field.name, "data"), FloatField()))
        )
    else:
...
def prepare_chart_data(chart, chart_data, timeline=True):
        ...
        if not has_series_group:
            ...
            for label, label_values in labels_dict.items():
                ...
                if chart.chart_type == "Line":
                    ...
                    dataset = {"label": label, "data": label_values, "backgroundColor": colors[i % 10]}
    ...
    data["options"] = {
        ...
        "scales": {
            "yAxes": [{"scaleLabel": {"display": True, "labelString": y_axis_label}}],
            "xAxes": [{"scaleLabel": {"display": True, "labelString": x_axis_label}}],
        }
        if chart.chart_type not in ["Pie", "Doughnut"]
        else {},
    }
        ...
        if key and (column in table_fields.keys() or filter_table_field in table_fields.keys()):
            ...
            if value == "__BLANK":
                filter_dict_table = filter_dict_table & Q(
                    Q(**{"data__{}__isnull".format(key): True}) | Q(**{"data__{}".format(key): ""})
                )
            else:
                if column_type in [
                    "float",
                    ...
                    filter_dict_table = filter_dict_table & Q(**{"data__{}".format(key): float(value)})
                else:
                    if column_type == "date":
                        if key_lookup == "relative":
                            ...
                            if relative_period == "weeks":
                                ...
                                date_start = date_start - relativedelta(days=(date_start.isoweekday() - 1) % 7, hours=2)
                            ...
                            filter_dict_table = filter_dict_table & Q(**{"data__{}__gte".format(column): date_start})
                            filter_dict_table = filter_dict_table & Q(
                                **{"data__{}__lt".format(column): date_start + relativedelta(**{relative_period: 1})}
                            )
                        else:
                            # filter_dict_table["data__{}".format(key)] = value
                            filter_dict_table = filter_dict_table & Q(**{"data__{}".format(key): value})
...
def get_card_data(request, card, table, preview=False):
    ...
    card_data = models.Entry.objects.filter(table=card.table).filter(filter_dict)

    if preview:
        card_data = card_data[:100]
    if card.data_column:
        data = card_data.aggregate(
            value=data_column_function(Cast(KeyTextTransform(card.data_column.name, "data"), FloatField()))
        )

    else:
        data = card_data.aggregate(value=data_column_function("id"))
